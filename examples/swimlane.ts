...
const submit = entity({ id: 'submit', label: '提交报销', tags: [{ id: 'expense', label: '报销流程' }] });
...
export const expense = swimlane({
  id: 'expense-flow', title: '报销流程：申请、审核与付款', width: 1000, headerWidth: 50,
  lanes: [
  ...
  nodes: [
    { entity: submit, role: submission, lane: 'employee', position: { x: 24, y: 40 }, size: { width: 180, height: 80 } },
    { entity: revise, role: submission, lane: 'employee', position: { x: 384, y: 40 }, size: { width: 180, height: 80 } },
    { entity: approve, role: review, lane: 'manager', position: { x: 24, y: 40 }, size: { width: 180, height: 80 } },
    { entity: pay, role: payment, lane: 'finance', position: { x: 384, y: 40 }, size: { width: 180, height: 80 } },
    { entity: archive, role: payment, lane: 'finance', position: { x: 704, y: 40 }, size: { width: 180, height: 80 } }
  ],
  relations: [
