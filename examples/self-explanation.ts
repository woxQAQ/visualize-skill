...
const skill = entity({
  id: 'skill', label: 'Skill', description: '指导 AI 组织内容与关系',
  tags: [{ id: 'authoring', label: '内容编写' }, { id: 'instructions', label: '操作手册' }]
});
const sdk = entity({
  id: 'sdk', label: 'TypeScript SDK', description: '声明身份、关系与阅读顺序',
  tags: [{ id: 'authoring', label: '内容编写' }, { id: 'typescript', label: 'TypeScript' }]
});
const coordinator = entity({
  id: 'coordinator', label: '生成入口', description: '组织检查、布局与渲染',
  tags: [{ id: 'build-time', label: '构建阶段' }, { id: 'orchestration', label: '调用编排' }]
});
const validator = entity({
  id: 'validator', label: '语义检查', description: '检查引用与调用生命周期',
  tags: [{ id: 'build-time', label: '构建阶段' }, { id: 'validation', label: '规则校验' }]
});
const layout = entity({
  id: 'layout', label: '图表布局', description: '应用几何声明与计算连线',
  tags: [{ id: 'build-time', label: '构建阶段' }, { id: 'geometry', label: '几何计算' }]
});
const renderer = entity({
  id: 'renderer', label: 'HTML 渲染', description: '绘制图表并组织节点属性',
  tags: [{ id: 'build-time', label: '构建阶段' }, { id: 'html', label: 'HTML' }, { id: 'svg', label: 'SVG' }]
});
const output = entity({
  id: 'output', label: '独立 HTML', description: '图表与结构化节点属性',
  tags: [{ id: 'offline', label: '离线阅读' }, { id: 'interactive', label: '节点交互' }]
});

export const overview = architecture({
  id: 'system-architecture', title: '系统架构：声明入口与生成分区',
  partitions: [{ id: 'generation', label: '生成过程', position: { x: 0, y: 220 }, size: { width: 908, height: 380 } }],
  nodes: [
    { entity: skill, role: guidance, position: { x: 0, y: 0 }, size: { width: 220, height: 88 } },
    { entity: sdk, role: declaration, position: { x: 344, y: 0 }, size: { width: 220, height: 88 } },
    { entity: coordinator, role: presentation, partition: 'generation', position: { x: 0, y: 0 }, size: { width: 220, height: 88 } },
    { entity: validator, role: checking, partition: 'generation', position: { x: 0, y: 190 } , size: { width: 220, height: 88 } },
    { entity: layout, role: presentation, partition: 'generation', position: { x: 320, y: 190 } , size: { width: 220, height: 88 } },
    { entity: renderer, role: presentation, partition: 'generation', position: { x: 640, y: 190 } , size: { width: 220, height: 88 } },
    { entity: output, role: artifact, position: { x: 664, y: 720 }, size: { width: 220, height: 88 } }
  ],
  relations: [
...
export const generation = sequence({
  id: 'generation-sequence', title: '生成时序：调用、执行与返回',
  participants: [
  ...
  steps: [
    ...
    { id: 'validation-result', kind: 'alternative', branches: [
      { label: '检查通过', steps: [
        ...
        { id: 'properties-ready', from: renderer, to: renderer, label: '属性已就绪', replyTo: 'assemble-properties' },
        { id: 'painted', from: renderer, to: coordinator, label: 'HTML 内容', replyTo: 'paint' },
