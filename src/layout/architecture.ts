...
export function layoutArchitecture(chart: ArchitectureChart, ctx: LayoutContext): ArchitectureScene {
  const partitions: PartitionLayout[] = chart.partitions.map(partition => {
    ...
    if (headerHeight + padding * 2 > height) fail('PARTITION_CONTENT_FIT', `diagram.${chart.id}.partitions.${partition.id}.size`, '分区没有足够的内容高度。', '增加分区声明的高度。');
    return { id: partition.id, x: margin + partition.position.x, y: margin + partition.position.y,
      width, height, title, headerHeight };
  });
  ...
  const nodes = chart.nodes.map(node => {
    ...
    if (partition && (box.x + box.width + padding > partition.x + partition.width || box.y + box.height + padding > partition.y + partition.height)) {
      fail('PARTITION_CONTENT_FIT', `diagram.${chart.id}.partitions.${partition.id}.size`, `节点 ${node.entity} 超出了分区 ${partition.id} 的内容区域。`, '调整分区 size，或节点的 position、size；系统保持声明的尺寸。');
    }
    return box;
  });
  ...
  for (const [index, partition] of partitions.entries()) {
    for (const other of [...partitions.slice(index + 1), ...ungrouped]) {
      if (overlaps(partition, other, 12)) fail('REGION_OVERLAP', `diagram.${chart.id}.partitions.${partition.id}.position`, `${partition.id} 与 ${other.id} 重叠或间距不足。`, '调整根节点或分区的 position、size，保留至少 12 像素间距。');
    }
  }
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      if (chart.nodes[i].partition === chart.nodes[j].partition && overlaps(nodes[i], nodes[j], 12)) {
        fail('NODE_OVERLAP', `diagram.${chart.id}.nodes.${nodes[j].id}.position`, `节点 ${nodes[i].id} 与 ${nodes[j].id} 重叠或间距不足。`, '调整 position 或 size，给节点及其连线留出至少 12 像素的间距。');
      }
    }
  }
