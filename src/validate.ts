...
export function validate(doc: SemanticDocument): SemanticDocument {
  ...
  const add: AddDiagnostic = (code, path, message, hint) => diagnostics.push({ code, path, message, hint });
  ...
  for (const entity of doc.entities) {
    for (const tag of entity.tags) {
      if (tags.has(tag.id) && tags.get(tag.id) !== tag.label) {
        add('TAG_IDENTITY_CONFLICT', `entity.${entity.id}.tags.${tag.id}`, `标签 ${tag.id} 在文档中有不同名称。`, '复用相同标签定义，或为不同分类使用不同标识。');
      }
      tags.set(tag.id, tag.label);
    }
  }
  if (!doc.blocks.length) add('EMPTY_DOCUMENT', 'document', '文档没有内容。', '追加 Markdown 或图表。');
  if (doc.roles.length > theme.palette.length) add('ROLE_CAPACITY', 'document.roles', `角色超过 ${theme.palette.length} 种，无法保持颜色可辨认。`, '合并相同职责，或将独立主题拆成不同文档。');
  for (const [index, block] of doc.blocks.entries()) {
    ...
    if (nodes.length > max) add('NODE_CAPACITY', path, `此类图最多支持 ${max} 个对象。`, '按系统边界或流程阶段拆图，保持文字可读。');
    const seen = new Set();
    const edge = (value: Relation) => {
      const p = `${path}.${value.id}`;
      if (seen.has(value.id)) add('DUPLICATE_RELATION', p, '关系或步骤标识重复。', '为每条关系、消息和条件分支设置唯一标识。');
      seen.add(value.id);
      for (const key of ['from', 'to'] as const) if (!ids.has(value[key])) add('UNKNOWN_ENDPOINT', `${p}.${key}`, `对象 ${value[key]} 未出现在当前图表中。`, '先将该对象加入 nodes 或 participants。');
    };
    if (chart.kind === 'architecture') {
      const partitionIds = new Set<string>();
      for (const partition of chart.partitions) {
        if (partitionIds.has(partition.id)) add('DUPLICATE_PARTITION', `${path}.partitions.${partition.id}`, '分区标识重复。', '为本图每个逻辑分区使用唯一标识。');
        partitionIds.add(partition.id);
        if (!chart.nodes.some(node => node.partition === partition.id)) add('EMPTY_PARTITION', `${path}.partitions.${partition.id}`, '分区没有包含节点。', '为节点声明 partition，或移除空分区。');
      }
    ...
    if (chart.kind === 'sequence') {
      ...
      values.forEach(edge);
      if (values.length > 16) add('RELATION_CAPACITY', path, '图表超过 16 条关系。', '按解释重点拆图，保留所有关系的含义。');
    }
  }
  const textSources = doc.blocks.flatMap((block, index) => block.kind === 'markdown'
    ? [{ content: block.content, path: `blocks[${index}].markdown` }] : []);
  for (const source of textSources) {
    for (const href of links(source.content)) {
      const [kind, id] = href.split(':');
      if (kind === 'entity' && !entities.has(id)) add('UNKNOWN_REFERENCE', source.path, `未找到对象 ${id}。`, '引用至少一张图中已声明的对象。');
      if (kind === 'diagram' && !diagrams.has(id)) add('UNKNOWN_REFERENCE', source.path, `未找到图表 ${id}。`, '检查 diagram: 后的图表标识。');
    }
  }
