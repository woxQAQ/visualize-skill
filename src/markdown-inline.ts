...
export const referenceId = (value: string) =>
  unescape(value).trim().replace(/\s+/g, " ").toLowerCase();
...
export function linkTarget(
  ...
  if (source[i] === "<") {
    ...
    while (i < source.length && !/\s/.test(source[i])) {
      if (source[i] === "\\" && punctuation.test(source[i + 1] ?? "")) {
        i += 2;
        continue;
      }
      if (source[i] === "(") depth++;
...
const literal = (token: Token): Inline =>
  token.kind === "delimiter" ? { kind: "text", text: token.marker.repeat(token.count) } : token;
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    for (let j = i - 1; j >= 0; j--) {
      ...
      if (
        ...
        (open.count + close.count) % 3 === 0 &&
        (open.count % 3 !== 0 || close.count % 3 !== 0)
      )
        continue;
      const count = Math.min(open.count, close.count) >= 2 ? 2 : 1;
      const wrapped: Inline = {
        kind: count === 2 ? "strong" : "emphasis",
        children: tokens.slice(j + 1, i).map(literal),
      };
      const nesting =
        1 +
        Math.max(
          ...
          ...tokens
            .slice(j + 1, i)
            .map((token) => (token.kind === "delimiter" ? 0 : (depths.get(token) ?? 0))),
        );
...
export function parseInline(
  ...
  definitions: ReadonlyMap<string, LinkTarget>,
  sourcePath: string,
): Inline[] {
  const source = lines.map((line) => line.text).join("\n");
  const path = (offset: number) =>
    `${sourcePath}.line[${lines[source.slice(0, offset).split("\n").length - 1].number}]`;
  const reject = (code: string, offset: number, message: string): never =>
    fail(
      code,
      path(offset),
      message,
      "使用支持的文字结构和链接；图表通过 diagram() 声明，HTML 示例放入代码块。",
    );
  const makeLink = (target: LinkTarget, children: readonly Inline[], offset: number): Inline => {
    if (
      (!/^(https?:\/\/|mailto:)/i.test(target.href) &&
        !/^(entity|diagram):[a-z][a-z0-9-]*$/.test(target.href)) ||
      // eslint-disable-next-line no-control-regex -- Link destinations must not contain ASCII controls or spaces.
      /[\u0000-\u0020\u007f]/.test(target.href)
    ) {
  ...
  const parse = (start: number, end: number, depth = 0, inLink = false): Inline[] => {
    ...
    for (let i = start; i < end;) {
      ...
      if (char === "`") {
        ...
        if (span) {
          tokens.push({ kind: "code", text: span.value });
          i = span.end;
          continue;
        }
      ...
      if (char === "[" || (char === "!" && source[i + 1] === "[")) {
        ...
        if (close < end) {
          ...
          if (target) {
            ...
            tokens.push(makeLink(target, parse(begin, close, depth + 1, true), i));
            i = next;
            continue;
          }
        }
      ...
      if (char === "<") {
        const auto = /^<([a-z][a-z\d+.-]*:[^<>\s]*|[^<>\s@]+@[^<>\s@]+\.[^<>\s@]+)>/i.exec(
          source.slice(i, end),
        );
        if (auto) {
          if (inLink) reject("MARKDOWN_LINK", i, "链接不能嵌套。");
          tokens.push(
            makeLink(
              { href: auto[1].includes(":") ? auto[1] : `mailto:${auto[1]}` },
              [{ kind: "text", text: auto[1] }],
              i,
            ),
          );
          i += auto[0].length;
        ...
        if (/^<(?:\/?[a-z][a-z\d-]*(?:\s|\/?>)|[!?])/i.test(source.slice(i, end)))
          reject("MARKDOWN_HTML", i, "报告不支持原始 HTML。");
      }
      if (char === "*" || char === "_") {
        let count = 1;
        while (i + count < end && source[i + count] === char) count++;
        const before = i === start ? "" : source[i - 1],
          after = i + count === end ? "" : source[i + count];
        const beforeSpace = !before || /\s/.test(before),
          afterSpace = !after || /\s/.test(after);
        const beforePunct = /[\p{P}\p{S}]/u.test(before),
          afterPunct = /[\p{P}\p{S}]/u.test(after);
        ...
        tokens.push({
          ...
          count,
          offset: i,
          open: left && (char !== "_" || !right || beforePunct),
          close: right && (char !== "_" || !left || afterPunct),
        });
        i += count;
        continue;
      }
      if (char === "\n") {
        ...
        if (last?.kind === "text")
          tokens[tokens.length - 1] = { kind: "text", text: last.text.trimEnd() };
        if (hard) tokens.push({ kind: "break" });
        else text("\n");
        i++;
        continue;
      }
      if (char === "&") {
        ...
        if (entity) {
          text(characterReference(entity[0]));
          i += entity[0].length;
          continue;
        }
      }
    ...
    return emphasis(tokens, (offset) =>
      reject("MARKDOWN_DEPTH", offset, "强调标记嵌套超过 32 层。"),
    );
  };
