...
try {
  ...
  if (values.help) {
    ...
    if (!isDocument(module.default)) fail('INVALID_DOCUMENT', 'export.default', '默认导出必须是 SDK 的 document() 结果。', '使用 export default document().markdown(...).diagram(...)。');
    if (values.check) {
      ...
      await rename(temp, output); temp = undefined;
      process.stdout.write(`${JSON.stringify({ ok: true, output })}\n`);
    }
  }
} catch (error) {
  if (temp) await rm(temp, { force: true });
  const diagnostics = error instanceof DiagnosticError ? error.diagnostics : [{ code: 'BUILD_ERROR', path: 'build', message: error instanceof Error ? error.message : String(error), hint: '检查输入路径、ES module 导出以及脚本错误。' }];
