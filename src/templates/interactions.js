...
(function initializeInteractions() {
  ...
  const restore = () => {
    if (current && placeholder) {
      current.querySelectorAll('[hidden]').forEach(element => { if (element instanceof HTMLElement) element.hidden = false; });
  ...
  const show = (id, chartId) => {
    ...
    const sections = /** @type {NodeListOf<HTMLElement>} */ (article.querySelectorAll('[data-detail-chart]'));
    ...
    const locations = [.../** @type {NodeListOf<HTMLElement>} */ (article.querySelectorAll('[data-location-chart]'))];
