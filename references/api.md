...
HTML 片段包含一张图、图例和当前停用的节点详情浮层，不包含完整页面、正文或外部资源。所有交互在当前图中完成。样式引用 Codex 提供的 CSS 主题变量，自动随应用配色和明暗模式更新，不需要重新生成图表。
...
详情展示对象标识、名称、摘要、标签、本图职责、分区或泳道归属，以及关联对象和关系。时序详情列出调用关系，返回仍在图中绘制。没有 `details`、`metadata` 或自由正文属性。

所有标识遵循 `^[a-z][a-z0-9-]*$`，例如 `order-api`。一张图内，同名对象、职责或标签必须有一致的定义；同一对象只能出现一次，可以连接多条关系。关系、消息和条件分支的标识在本图内唯一。参数对象只接受文档列出的字段。

## 容器宽度

`compile(diagram)` 保留声明的宽版几何结果。`render(diagram)` 同时准备 288、432、576、704 像素的紧凑布局，只生成比宽版更窄的档位。CSS 容器查询根据图表可用宽度选择一档，不需要 JavaScript 重算布局，也不把整张 SVG 缩小。片段两侧各留 16 像素，因此支持的最小容器宽度为 320 像素。图表高度随内容展开，不限制区域高度，也不引入内部滚动。

架构节点与分区按声明坐标的从上到下、从左到右顺序换行；分区归属不变。泳道内活动换行，必要时增加泳道高度。节点保留 14 像素标题与 12 像素摘要，先重新换行计算高度，再重新布线。长关系标签无处放置时，连线上显示编号，完整标签和端点显示在关系图例中。

时序图收紧参与者间距并重新换行；名称或消息仍放不下时，使用逐条调用视图。该视图重复显示每条消息的发送方和接收方，保留全部调用、返回配对与互斥分支，不绘制持续的生命线和执行条。容器变宽后恢复完整时序图。

`--check` 同时检查宽版和紧凑布局，不写出文件。

## 主题

颜色 token 是按用途命名的 CSS 变量。文字使用 `--foreground`，次要文字与连线使用 `--muted-foreground`，背景使用 `--background`，边界使用 `--border`。节点边框与图例使用六个 `--viz-series-*` 分类色；节点底色由 10% 分类色与 90% 背景色混合得到。分类色并不保证始终是某个固定色相。

Codex 注入这些变量并同步主题变化；生成器只输出变量引用，不读取机器的明暗设置，不增加主题切换脚本或锁定浅色。HTML 片段的颜色依赖宿主主题环境，浏览器预览也需要提供同一套变量。几何尺寸与文字排版不受主题切换影响。

## 交互

节点详情目前停用，点击节点或按 Enter 不打开详情。悬停和键盘聚焦仍强调相邻关系。浮层、资料与关联定位实现保留，以下详情操作仅在恢复启用后生效。

| 操作                              | 结果                                                         |
| --------------------------------- | ------------------------------------------------------------ |
| 悬停或键盘聚焦节点                | 强调本节点、直接相邻节点和相连关系，弱化其余部分；只扩展一层 |
| 移开鼠标并移走键盘焦点            | 恢复常态，保留已有定位标记                                   |
| 点击节点或按 Enter                | 在节点旁显示详情浮层，图表仍可操作                           |
| 点击详情中的关联节点              | 关闭详情，定位当前布局中的目标节点并聚焦、强调               |
| 点击图中空白                      | 清除定位标记                                                 |
| 点击关闭按钮、浮层外部或按 Escape | 关闭详情；从浮层内关闭时恢复入口焦点                         |

详情使用浏览器原生 `popover`，即不阻止图表操作的小浮层，不显示蒙层或限制键盘焦点。浮层优先放在节点侧边，空间不足时放在上方或下方，并保持在当前嵌入容器内；点击其他节点直接切换详情。

交互脚本不改变宿主页面、地址片段或浏览器历史，不请求网络。图表随容器宽度选择布局，支持宽度内不出现横向滚动条，文字字号不缩小。
