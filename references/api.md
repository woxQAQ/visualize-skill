...
内容脚本从 Skill 目录的 `src/index.ts` 导入接口。生成与展示方式见 [SKILL.md](../SKILL.md)。
...
| `render(diagram)`       | HTML 片段字符串                   | 执行完整检查并渲染，继承宿主主题，不写文件 |
| `renderPage(diagram)`   | 完整 HTML 页面字符串              | 包装相同片段并提供默认主题，不写文件       |
...
声明和语义数据均不可变，可以 JSON 序列化；`compile()`、`render()` 和 `renderPage()` 只接受 SDK 创建的图表，不接受普通 JSON、数组或正文。直接默认导出一张图：
...
两种输出均展示一张图及其图例，内嵌样式和交互，无外部资源。`renderPage()` 生成可以直接在浏览器打开的完整页面，包含标题、移动端 viewport 和默认明暗配色；`render()` 生成继承宿主 CSS 主题变量的片段，用于嵌入。图表几何与交互完全相同。
...
`role()` 只有必填的 `id` 和 `label`。每张图最多 6 种职责，按标识排序对应主题的 `--viz-series-1` 到 `--viz-series-6` 分类色。不同图中的职责集合不同，颜色可能重新分配；颜色不表示跨图身份。
...
| `return`   | 返回或回传结果       | `--muted-foreground` | 1.5  | 短虚线，线段 5、间隔 4，开口箭头 |

`return` 意为返回样式，使用次要文字色和开口箭头；其余预设默认使用实心箭头。架构关系的方向仍由 `from`、`to` 决定，不会自动反转端点。

尺寸是缩放前的画布单位。实际色值随当前主题变化；`security` 使用主题红色，`return` 使用次要文字色，其余分类色不绑定固定色相。关系文字应说明实际含义，不能只靠颜色表达语义。悬停时保留预设颜色和线型，线宽增加 1、文字加粗，并强调两个端点。
...
独立页面在 `src/templates/page.css` 中定义完整的默认配色，通过 `prefers-color-scheme` 跟随浏览器明暗偏好，无主题监听脚本。页面外壳负责根元素、背景和内容宽度；图表内部继续使用同一套颜色变量。

嵌入片段不写入默认配色或全局页面样式，由宿主提供主题变量；Codex 可直接提供这些变量，具体展示方式见 [Codex 展示](codex.md)。片段不适合直接作为无主题的浏览器页面交付，应改用 `renderPage()` 或 `--format page`。几何尺寸与文字排版不受主题变化影响。
...
node "<skill>/src/cli.ts" diagram.ts --format fragment -o fragment.html
node "<skill>/src/cli.ts" diagram.ts --check
```

CLI 接收一个内容脚本及 `-o` / `--output`、`--format page|fragment`、`--check`、`-h` / `--help`。`page` 表示独立页面，`fragment` 表示嵌入片段；省略 `--format` 仍输出片段。未知格式返回 `INVALID_FORMAT`，在加载内容脚本之前拒绝。成功时 stdout 输出 JSON：检查为 `{ "ok": true, "diagram": "图表标识" }`，生成结果为 `{ "ok": true, "output": "输出绝对路径" }`。失败时 stderr 输出 `{ "ok": false, "diagnostics": [...] }`，退出码为 1。输入脚本日志应写 stderr。
