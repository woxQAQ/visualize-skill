...
`compile()` 的宽版几何遵循下述坐标与尺寸规则。画布四周留 32 像素。泳道从画布内容原点依次向下紧邻排列，共用相同宽度，泳道高度严格采用声明值。左侧标题栏由图表级 `headerWidth` 控制，默认 144，标题左右各留 16 像素。所有泳道共用这一宽度，以保持标题栏边界对齐。
...
每张图最多 12 个节点、16 条关系。节点名称最多 3 行，摘要最多 4 行，仍须满足声明尺寸。画布最大宽 1280、高 2600，包含四周边距，因此 `width` 最大为 1216，所有泳道高度之和最多为 2536。窄屏中活动按声明坐标的阅读顺序在所属泳道内换行，泳道随内容增高，标题栏宽度最多为 80 像素；文字字号保持不变，详见[容器宽度](api.md#容器宽度)。
