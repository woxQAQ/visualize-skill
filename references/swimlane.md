...
| `relations`   | 流转关系，每条包含 `id`、`from`、`to`、`label` 和可选的 `variant`、`fromSide`、`toSide`；允许空数组 |

除 `headerWidth` 外，以上字段均必填。至少声明一条泳道和一个节点；每条泳道至少包含一个节点。泳道标识在当前图中唯一，名称为最多 48 字符的单行短文本，标题实际最多排 3 行。每个活动必须属于一条已声明的泳道，没有根节点或嵌套泳道。

关系的 `variant` 默认是 `default`，预设定义见[关系样式](api.md#关系样式)。通过 `fromSide` 和 `toSide` 指定节点的连接边，规则见[关系连接边](api.md#关系连接边)。指定边后仍须在泳道内容区域内完成布线与标签放置，无法满足时返回诊断。

泳道本身不进入共享对象表，没有 `role`、标签或详情，不能作为连线端点。一个负责方执行多个活动时，为各活动声明独立实体，并使用同一个 `lane`。节点详情展示所属泳道以及流入、流出关系，支持本图内关联节点定位。
...
`compile()` 的几何遵循下述坐标与尺寸规则。画布四周留 32 像素。泳道从画布内容原点依次向下紧邻排列，共用相同宽度，泳道高度严格采用声明值。左侧标题栏由图表级 `headerWidth` 控制，默认 144，标题左右各留 16 像素。所有泳道共用这一宽度，以保持标题栏边界对齐。
...
跨泳道的纵向主流程按列中心排布。各泳道内容区的横向原点相同，因此同列节点应满足 `position.x + size.width / 2` 相等，而不只是 `position.x` 相等。例如，上方节点 `x: 40, width: 180` 的中心为 130；下方节点宽 200 时应取 `x: 30`，两端中点才能垂直连接。同一泳道内横向直连的节点则保持 `position.y + size.height / 2` 相等。需要比较不同泳道的纵坐标时，先加上各泳道内容区原点的偏移。

在内容脚本中复用列中心值，再用“列中心减去半个节点宽度”计算 `position.x`，可以在调整宽度时保持对齐。以主要流程为准安排节点，分支、回退和避障仍可转折。
...
每张图最多 12 个节点、16 条关系。节点名称最多 3 行，摘要最多 4 行，仍须满足声明尺寸。画布最大宽 1280、高 2600，包含四周边距，因此 `width` 最大为 1216，所有泳道高度之和最多为 2536。窄容器中整张图等比例缩小，保留泳道、标题栏和活动的原始布局，文字一同缩放，详见[容器宽度](api.md#容器宽度)。
