...
`variant` 与 `relations` 共用 `RelationVariant` 类型和六种预设：`default`、`emphasis`、`security`、`dashed`、`external`、`return`。省略时使用 `default`，调用和返回分别声明，不自动继承；条件片段不接受 `variant`。
...
消息端点可传对象或标识字符串，两端都必须出现在当前图的参与者中。调用默认使用实心箭头，箭头和线型可通过 `variant` 改变。返回始终使用开口箭头和短虚线；接收调用时开始执行区间，返回时结束。调用必须配对返回，返回端点与调用相反。先返回内层调用，再返回外层调用；等待同步调用的参与者不能继续发出其他调用。`from` 和 `to` 相同表示自调用，仍需要返回消息。

`return` 表示返回样式，使用次要文字色、短虚线和开口箭头。它只改变外观，仍需 `replyTo` 指定所返回的调用；省略 `replyTo` 的消息仍按同步调用处理。
...
每张图最多 6 个参与者、32 条调用与返回消息，消息数统计所有分支。同一参与者的执行最多嵌套 4 层，条件片段最多嵌套 3 层。参与者名称最多排 3 行，并须放入声明尺寸。声明的画布最大宽 1280、高 2600。容器变窄时整张时序图等比例缩小，始终保留参与者排列、消息顺序、生命线、执行条、返回配对和互斥分支，详见[容器宽度](api.md#容器宽度)。
