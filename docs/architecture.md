...
| `layout/sequence.ts`     | 时序语义到几何结果                 | 参与者、同步执行条、异步消息、响应与自调用               |
...
图表具有不同关系模型：架构图使用 `relations` 与独立的 `partitions`，泳道图使用 `relations` 与独立的 `lanes`，时序图使用有序 `messages`。泳道关系表达活动流转，多个出边可以附带条件标签，回退仍是方向关系；当前不校验互斥条件、并行执行或 BPMN 网关语义。时序 `Message` 和 `MessageInput` 是独立接口，不继承 `Relation`，不使用调用、响应或分支的联合类型，也不包含 `kind`、`fromSide`、`toSide`。`MessageVariant` 的五种预设由消息专属的入口校验，省略时为 `default`。`return` 表示响应，必须通过 `replyTo` 关联先前消息；其他预设不能声明 `replyTo`。条件分支不再属于时序模型。
...
`pnpm typecheck` 运行 TypeScript 严格检查，`pnpm lint` 与 `pnpm fmt:check` 检查代码。通过 CLI 生成示例验证声明、语义校验、布局与 HTML 输出的完整路径；不添加单元测试。`pnpm example`、`pnpm example:sequence` 与 `pnpm example:swimlane` 分别生成三种图表片段，`examples/message-variants.ts` 展示五种消息预设、异步响应与无需响应的通知。
