...
架构自动布局在分区内部和根级别都按依赖自上而下分层，来自 `placeBoxes()` 默认的 `horizontal: false`。公开架构声明目前没有方向选项；作者通过节点与分区的 `position` 选择横向、纵向或混合组织方式。默认排布只是初稿，不代表架构关系应画成纵向流程；应结合职责之间的并列或层级关系及实际展示宽度安排位置。
