...
`);
  ...
  await access(join(installed, 'dist/templates/document.html'));
  const css = await readFile(join(installed, 'dist/templates/report.css'), 'utf8');
  const script = await readFile(join(installed, 'dist/templates/interactions.js'), 'utf8');
  assert.ok(html.includes(`<style>${css}</style>`));
  assert.ok(html.includes(`<script data-visualize-interaction>${script}</script>`));
  assert.doesNotMatch(script, /^\s*(?:import|export)\b/m);
  assert.doesNotMatch(html, /<link|\bsrc=/);
  await assert.rejects(access(join(installed, 'dist/interactions.js')));
  ...
  const standalone = await mkdtemp(join(tmpdir(), 'visualize-standalone-'));
  t.after(() => rm(standalone, { recursive: true, force: true }));
  await exec('tar', ['-xzf', tarball, '-C', standalone, '--strip-components=1']);
  await assert.rejects(access(join(standalone, 'node_modules')));
  const output = join(standalone, 'standalone.html');
  await exec(process.execPath, [join(standalone, 'dist/cli.js'), join(standalone, 'examples/self-explanation.ts'), '-o', output], { cwd: tmpdir() });
  const generated = await readFile(output, 'utf8');
  assert.match(generated, /用图表探索这套系统/);
  assert.match(generated, /data-entity-detail="details-sdk"/);
